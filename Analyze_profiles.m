function Analyze_profiles(Profiles, analysis_save_path)
% Analyze_profiles(Profiles, analysis_save_path)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Created by Greg Salomons
%    Analyze_profiles Plots profile figures and saves data to excel files.
%
%  Input Arguments
%     analysis_save_path =  the path for saving the profile spreadsheets
%     Profiles   = A table containing measured and calculated Profile data
...
%% Select Profiles
% Convert profile depths to numbers
Profiles.Depth = cell2mat(Profiles.Depth);

%Convert Energy to a numerical variable and sort Data
expr = '^\s*(\d+)\s*MeV\s*$';
Energy_func = @(x) str2double(regexprep(x,expr,'$1'));
Profiles.EnergySort = cell2mat(cellfun(Energy_func,Profiles.Energy,'UniformOutput',false));

sort_categories = {'EnergySort','EquivSquare','Depth','Source',};
Profiles = sortrows(Profiles,sort_categories);
 
% Set categories and find profile groups
group_categories = {'Energy';'Applicator';'FieldSize';'Depth'};
[GroupIndex,Groups] = findgroups(Profiles(:,group_categories));

% Select only profiles that have both Calculated and Measured
Compare_on = {'Source'};
for i = 1:height(Groups)
    Groups{i,'NumSources'} = size(unique(Profiles{GroupIndex==i,Compare_on}),1);
end
ProfileSelection = find(Groups.NumSources > 1);
SelectedProfileGroups = Groups(ProfileSelection,:);
SelectedProfileGroups.Index = ProfileSelection;

%% Define profile group labels for plotting and saving
ProfileGroupVariables = {'Distance', 'Dose', 'Source', 'Linac', 'Algorithm'};

group_labels = struct;
% Define an excel directory (This is used for saving data to excel files)
group_labels(1).Name = 'Directory';
group_labels(1).Elements = {'Energy'};
...
%% Define profile data labels for plotting and saving
data_labels = struct;
...
for n = 1:size(group_labels,2)
    SelectedProfileGroups(:,group_labels(n).Name) = MakeColumn(SelectedProfileGroups, group_labels(n));
end

for i = 1:height(SelectedProfileGroups)
    group = table2struct(SelectedProfileGroups(i,:));
    
    group_data = Profiles(GroupIndex==group.Index,ProfileGroupVariables);
    % Add data titles
...
%% Save Profiles agregate data
Profiles_data_analysis = Profiles;
Profiles_data_analysis.EnergySort = [];
% Remove PDD related data columns
Profiles_data_analysis.DepthShift = [];
Profiles_data_analysis.SurfaceDose = [];
Profiles_data_analysis.Buildup_95 = [];
Profiles_data_analysis.R100 = [];
Profiles_data_analysis.R95 = [];
Profiles_data_analysis.R90 = [];
Profiles_data_analysis.R80 = [];
Profiles_data_analysis.R50 = [];
% Remove data curves
Profiles_data_analysis.Distance = [];
Profiles_data_analysis.Dose = [];
% Remove data curves
Profiles_data_analysis.InsertShape = [];
Profiles_data_analysis.Offset = [];

%Save the profile analysis data to a spreadsheet
filename = [analysis_save_path '\' 'Profile Analysis.xls'];
writetable(Profiles_data_analysis,filename)