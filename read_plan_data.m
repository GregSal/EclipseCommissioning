...
%
if (nargout ==3)    % Plan Data and Dose Data Desired
    ...
    for i=1:size(Dose_files_found,1)
        ...
        if strcmp(Test_Plan_file_ID,Plan_Info.SOPInstanceUID)
            % If it matches save this file name and the field number it
            % references
            Dose_file_list{dose_file_index} = Dose_files_found{i}; %#ok<AGROW>
            try
                Field_ID = Dose_Info.ReferencedRTPlanSequence.Item_1.ReferencedFractionGroupSequence.Item_1.ReferencedBeamSequence.Item_1.ReferencedBeamNumber;
            catch ME
                Field_ID = 0;
            end
            Beam_Reference(dose_file_index) = Field_ID; %#ok<AGROW>
            dose_file_index = dose_file_index + 1;
        end
    end
...
for i = 1:Plan_data.Number_of_Beams
    ...
    Plan_data.Beams(i).Field_Number = Beam_data.BeamNumber;
    ...
    try
        ...
        %If not valid, return empty variables ?Electrons?
        % TODO find out how to determine SSD for electrons  (dose file??)
    ...
    if (strcmp(Plan_data.Beams(i).Radiation_Type,'ELECTRON'))  % If the Radiation Type is Electrons
        ...
        if (isfield(Beam_data, 'BlockSequence')) % test for presence of Electron insert
            ...
            points = size(Shape);
            Plan_data.Beams(i).InsertShape=reshape(Shape,2,[])';
            Plan_data.Beams(i).InsertName=Beam_data.BlockSequence.Item_1.BlockName;
        end
    else
        %% Get MLC Data
        % Check for MLCs
        ...
        if Blocks >0
            for B = 1:Blocks
                ...
                Plan_data.Beams(i).block(B).Shape=reshape(Shape,2,[]);
            end
        end
    end
end
